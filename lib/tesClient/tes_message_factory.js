...
exports.buildAccountInfoCapnp = buildAccountInfoCapnp;
exports.buildAccountCredentials = buildAccountCredentials;
exports.buildLogonCapnp = buildLogonCapnp;
exports.buildAccountInfo = buildAccountInfo;
exports.buildHeartbeatCapnp = buildHeartbeatCapnp;
exports.buildLogonAckJs = buildLogonAckJs;
exports.buildLogoffAckJs = buildLogoffAckJs;
exports.deserializeCapnp = deserializeCapnp;
exports.buildGetAccountBalancesCapnp = buildGetAccountBalancesCapnp;

var _common_types = require("./common_types");

function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }

// tes_message_factory.js
// external dependency imports
var capnp = require("capnp"); // load capnp schema


var msgs_capnp = require("../../lib/CommunicationProtocol/TradeMessage.capnp");

var exch_capnp = require("../../lib/CommunicationProtocol/Exchanges.capnp"); // internal dependency imports
...
function buildAccountInfoCapnp(accountID) {
  var accountInfo = new _common_types.AccountInfo(1, "kraken", "exchange", "123", "456");
  console.log(accountInfo);
  var logon_capnp = capnp.serialize(msgs_capnp.AccountInfo, accountInfo);
  console.log(logon_capnp);
  var logonJs = capnp.parse(msgs_capnp.AccountInfo, logon_capnp);
  console.log(logonJs);
...
var TradeMessage = function TradeMessage(type) {
  _classCallCheck(this, TradeMessage);

  this.type = type;
};

var Request = function Request(clientId, senderCompId, body) {
...
var Logon = function Logon(credentials) {
  _classCallCheck(this, Logon);

  this.credentials = credentials;
};

var Body = function Body(bodyType, body) {
...
var Type = function Type(tradeMessageType, tradeMessage) {
...
function buildMessageBody(bodyType, body) {
  switch (bodyType) {
    ...
    case "getAccountBalances":
      return {
        getAccountBalances: body
      };

    case "getOpenPositions":
      return {
        getOpenPositions: body
      };

    case "getWorkingOrders":
      return {
        getWorkingOrders: body
      };

    case "getCompletedOrders":
      return {
        getCompletedOrders: body
      };

    case "getExchangeProperties":
      return {
        getExchangeProperties: body
      };
  }
}
...
function buildAccountInfo(accountId, exchange, accountType, exchangeAccountId, exchangeClientId) {
  var accountInfo = {
    accountID: accountId,
    exchange: 'undefined',
...
function buildLogonCapnp(clientId, senderCompId, credentials) {
  var logon = buildLogon(credentials);
  var request = buildRequest(clientId, senderCompId, buildMessageBody("logon", logon)); // return capnp.serialize(msgs_capnp.Request, request);

  var tradeMessage = buildTradeMessage(buildType("request", request));
  return capnp.serialize(msgs_capnp.TradeMessage, tradeMessage);
}

function buildHeartbeatCapnp(clientId, senderCompId) {
  var request = buildRequest(clientId, senderCompId, buildMessageBody("heartbeat"));
  var tradeMessage = buildTradeMessage(buildType("request", request));
  return capnp.serialize(msgs_capnp.TradeMessage, tradeMessage);
}

function buildLogoffCapnp(clientId, senderCompId) {
  var request = buildRequest(clientId, senderCompId, buildMessageBody("logoff")); // return capnp.serialize(msgs_capnp.Request, request);
...
function buildGetAccountBalancesCapnp(clientId, senderCompId, accountInfo) {
  var getAccountBalances = buildGetAccountBalances(accountInfo);
  var request = buildRequest(clientId, senderCompId, buildMessageBody("getAccountBalances", getAccountBalances));
  var tradeMessage = buildTradeMessage(buildType("request", request));
  return capnp.serialize(msgs_capnp.TradeMessage, tradeMessage);
}
...
function buildPlaceOrder(placeOrderArguments) {
  return {
    accountInfo: placeOrderArguments.accountInfo,
...
function buildPlaceOrderCapnp(clientId, senderCompId, placeOrderArguments) {
  var getAccountBalances = buildPlaceOrder(placeOrderArguments);
  var request = buildRequest(clientId, senderCompId, buildMessageBody("getAccountBalances", getAccountBalances));
  var tradeMessage = buildTradeMessage(buildType("request", request));
  return capnp.serialize(msgs_capnp.TradeMessage, tradeMessage);
}
...
function buildLogonAckJs(logonAck) {
  ...
  */
  if (Boolean(logonAck.success)) {// console.debug('Logon success', 'status:', 'logon_success');
  } else {} // console.debug('Logon failure', 'status:', 'logon_failure');
    // console.debug('logon response: ', logonAck.message);


  var clientAccounts = logonAck.clientAccounts; // console.debug('client accounts:', clientAccounts);
...
function buildLogoffAckJs(logoffAck) {
  ...
  */
  if (logoffAck.success) {// console.debug('Logoff success.', 'status', 'logoff_success');
  } else {} // console.debug('Logoff failure.', 'status', 'logoff_failure');
...
function buildAccountBalancesReportJs(accountBalancesReport) {
  return [accountBalancesReport.accountInfo, accountBalancesReport.balances];
}

function buildAccountDataReportJs(accountDataReport) {
  return [accountDataReport.accountInfo, accountDataReport.balances, accountDataReport.openPositions, accountDataReport.orders];
}

function buildExecutionReportJs(executionReport) {
  return [executionReport.orderID, executionReport.clientOrderID, executionReport.clientOrderLinkID, executionReport.exchangeOrderID, executionReport.accountInfo, executionReport.symbol, executionReport.side, executionReport.orderType, executionReport.quantity, executionReport.price, executionReport.timeInForce, executionReport.leverageType, executionReport.leverage, executionReport.orderStatus, executionReport.filledQuantity, executionReport.avgFillPrice, executionReport.rejectionReason];
}
...
function buildOpenPositionsReportJs(openPositionReport) {
  ...
  */
  ...
  return new OpenPositionsReport(accountInfo = build_js_account_info_from_capnp(openPositionReport.accountInfo), openPositions = open_pos);
