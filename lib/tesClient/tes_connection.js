...
exports.createAndBindTesSockets = createAndBindTesSockets;
exports.messageHandlerCallbackObjectFactory = messageHandlerCallbackObjectFactory;

var _TesSocket = _interopRequireDefault(require("./sockets/TesSocket"));

var _BackendSocket = _interopRequireDefault(require("./sockets/BackendSocket"));

var _tes_message_factory = require("./tes_message_factory");
...
function messageHandlerCallback(binaryMessage, callbackObject) {
  var tradeMsgObj = capnp.parse(msgs_capnp.TradeMessage, binaryMessage);
  var messageObject = tradeMsgObj.type.response.body;
  var messageType = Object.keys(messageObject)[0];
  callbackObject[messageType]((0, _tes_message_factory.deserializeCapnp)(messageType, messageObject[messageType]));
}

function createAndBindTesSockets(curveServerKey, tesConnectionString, backendConnectionString, callbackObject) {
  var tes_socket = new _TesSocket.default({
    curve_server_key: curveServerKey,
    socket_endpoint: tesConnectionString
  });
  tes_socket.activate({
    onMessage: function onMessage(message) {
      return messageHandlerCallback(message, callbackObject);
    }
  });
  var backend_socket = new _BackendSocket.default({
    tes_socket: tes_socket,
